 ...
 * This class ensures only one instance is used throughout the application.
 */
public final class AwesomePasswordChecker {

    // Constants to replace magic numbers
    private static final int MASK_ARRAY_SIZE = 28;
    private static final int LOWERCASE_MASK = 2;
    private static final int UPPERCASE_MASK = 4;
    private static final int DIGIT_MASK = 5;
    private static final int SPECIAL_CHAR_MASK = 6;
    private static final int OTHER_MASK = 7;
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(is))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                for (int i = 0; i < values.length; ++i) {
                    try {
                        center[i] = Double.parseDouble(values[i].replace(",", ".")); // Remplacer les virgules
    ...
    public int[] maskAff(final String password) {
        ...
        for (int i = 0; i < limit; ++i) {
            ...
            if (Character.isLowerCase(c)) {
                maskArray[i] = LOWERCASE_MASK;
            } else if (Character.isUpperCase(c)) {
                maskArray[i] = UPPERCASE_MASK;
            } else if (Character.isDigit(c)) {
                maskArray[i] = DIGIT_MASK;
            } else if ("<>-?.!/".contains(String.valueOf(c))) {
                maskArray[i] = SPECIAL_CHAR_MASK;
            } else {
                maskArray[i] = OTHER_MASK;
            }
        }
        return maskArray;
    }
...
public static String computeMd5(final String input) {
    try {
        ...
        // Step 2: Compute number of blocks and total length
        ...
        // Step 3: Append length in bits as a 64-bit little-endian integer
        ...
        // Step 5: Initialize MD5 hash values
